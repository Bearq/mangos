    ...
    m_sourceUnit(owner), m_navMesh(NULL), m_navMeshQuery(NULL)
{
    ...
    DEBUG_FILTER_LOG(LOG_FILTER_PATHFINDING, "++ PathInfo::PathInfo for %s \n", m_sourceUnit->GetObjectGuid().GetString().c_str());
                      ...
                      bool useStraightPath, bool forceDest)
{
    ...
    DEBUG_FILTER_LOG(LOG_FILTER_PATHFINDING, "++ PathInfo::Update() for %s \n", m_sourceUnit->GetObjectGuid().GetString().c_str());
...
void PathInfo::BuildPolyPath(PathNode startPos, PathNode endPos)
{
    ...
    else if (startPolyFound && !endPolyFound)
    {
        ...
        if (DT_SUCCESS != m_navMeshQuery->closestPointOnPoly(suffixStartPoly, endPoint, suffixEndPoint))
        {
            ...
            sLog.outError("%s's Path Build failed: invalid polyRef in path", m_sourceUnit->GetObjectGuid().GetString().c_str());
        ...
        if (!suffixPolyLength || dtResult != DT_SUCCESS)
        {
            ...
            sLog.outError("%s's Path Build failed: 0 length path", m_sourceUnit->GetObjectGuid().GetString().c_str());
        }
